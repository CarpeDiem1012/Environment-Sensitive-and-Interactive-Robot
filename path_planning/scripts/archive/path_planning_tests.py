...
class PathPlanning():
    def __init__(self, ros_enabled, husky_size, start_position_husky, occupancy_map, geometric_distance=True, animation=False):
        ...
        self.coverage = 0
        self.occupancy_grid = occupancy_map
        self.geometric_distance = geometric_distance

        if ros_enabled == False:
            self.cv_image = occupancy_map
...
############### Receiving position and scaling factor####################
    ...
    def callback_localization(self, data):
        ...
        if self.length > 10:
            ...
            self.length = int(0.6667*68)
            self.husky_size = (int(68*0.7*0.6667), int(0.6667*68))
        
        if abs(x) > 10 or abs(y) > 10:
            self.start_position_husky = (314,377)
        else:
...
############### START PLANNING SEQUENCE ####################
    ...
    def callback_image(self, data):
        ...
        path = self.google_TSP()
        # path = self.mlrose_tsp_opt()

        rospy.loginfo("[Debug] path.shape = %s", path.shape)
        rospy.loginfo("[Debug] path.type = %s", type(path))
        self.publish(path)
...
############# Publish path ###################
    ...
    def publish(self, input):
        input = np.array(input, dtype='int32').reshape(-1)
        rospy.loginfo("[Debug] msg.shape = %s", input.shape)
        msg = Int32MultiArray()
        msg.data = input
...
####################################################
    ...
    def make_distance_matrix2(self):
        ...
        rospy.loginfo('AMOUNT OF WAYPOINTS TO VISIT: '+str(self.distance_matrix.shape[0]))
        for i in tqdm(range(self.search_grid_list.shape[0])):
...
####################################################
    ...
    def make_path_list(self, best_state):
        ...
        for state in best_state:
            path_list.append(list(self.search_grid_list[state]))
    ...
    def make_geometrix_path_list(self, path_list):
        ...
        for waypoint in path_list:
            ...
            if self.geometric_distance:
                geo_position = centered_position/(int(self.husky_size[0])*1.340)
            else:
...
######################################################
    ...
    def google_TSP(self):
        ...
        search_parameters.solution_limit = 10**9
    ...
    def depot_number(self):
        ...
        grid_list = self.search_grid_list
    ...
    def hand_made_solver(self):
        ...
        index_list.append(list([depot_number]))
...
if __name__ == "__main__":
    ...
    occupancy_map = np.array(im)
    occupancy_map = cv2.cvtColor(occupancy_map, cv2.COLOR_BGR2GRAY)
    occupancy_map[142:340,187:383] = 34
    occupancy_map = occupancy_map[22:332,14:552]
    occupancy_map[occupancy_map == 255] = 0
    occupancy_map[occupancy_map == 34] = 255
    occupancy_map[occupancy_map == 23] = 255

    husky_size = (14,20)
    start_position_husky = (int(occupancy_map.shape[0]//2),144)
    ros_enabled = False
